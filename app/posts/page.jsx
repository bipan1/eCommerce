...
export default function CreatePost() {
    ...
    const createPostFailure = () => toast.error("Failed to create a post.")
    ...
    const handleCreatePost = async (values) => {
        ...
        try {
            if (images.length > 0) {
                ...
                formattedUrls = presignedUrls.map(url => url.split('?')[0])
                await Promise.all(images.map((file, index) => uploadFileToS3(file.originFileObj, presignedUrls[index])));
            }

            const data = { ...values, images: images.length > 0 ? formattedUrls : [], userId: session.user.id };
